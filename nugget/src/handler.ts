...
export function resetPolicyWarningForTests(): void {
...
export class AIHandler {
  ...
  constructor(private opts: HandlerOptions) {
    ...
    if (!opts.policy && !opts.silencePolicyWarning && !warnedNoPolicy) {
      ...
      globalThis.console?.warn?.(
        '[ai-nugget] No GovernancePolicy configured — every provider/model is allowed. ' +
  ...
  async *stream(conn: Connection, req: ChatRequest): AsyncIterable<StreamEvent> {
    ...
    if (!preflight.ok) return yield* this.failStream(callId, conn, req, startedAt, 1, preflight.error);
  ...
  ): Promise<{ ok: true; resolved: ResolvedConnection } | { ok: false; error: AIError }> {
  ...
  private async *failStream(callId: string, conn: Connection, req: ChatRequest, startedAt: number, attempts: number, error: AIError): AsyncGenerator<StreamEvent> {
  ...
  private async acquire(signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      if (this.lastStarted === reserved) this.lastStarted = previousLastStarted;
      throw error;
    }
  }
...
function redactDeep(value: unknown, redact: (text: string) => string): unknown {
  ...
  if (Array.isArray(value)) return value.map((entry) => redactDeep(entry, redact));
 ...
 */
function safeConnection(conn: Connection): Connection {
  return { ...conn, ...safeKeyRef(conn) };
}

function safeKeyRef(conn: Connection): { keyRef?: Connection['keyRef'] } {
  if (conn.keyRef?.kind !== 'literal') return conn.keyRef === undefined ? {} : { keyRef: conn.keyRef };
 ...
 * header.
 */
function withIdempotencyKey(resolved: ResolvedConnection, callId: string): ResolvedConnection {
  const profile = profileFor(resolved.provider, resolved.baseUrl);
  if (!profile.quirks?.supportsIdempotencyKey) return resolved;
  if (Object.keys(resolved.headers).some((name) => name.toLowerCase() === 'idempotency-key')) return resolved;
