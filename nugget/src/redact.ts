 ...
 */
export const LABELED_SECRET_PATTERNS: RegExp[] = [
  new RegExp(
    ...
    String.raw`(?<![A-Za-z0-9])(api[_-]?key|apikey|secret[_-]?access[_-]?key|access[_-]?token|auth[_-]?token|session[_-]?token|client[_-]?secret|refresh[_-]?token|private[_-]?key|password|passwd)` +
      ...
      String.raw`(["']?\s*[:=]\s*|\s+)` +
      String.raw`(["']?)([A-Za-z0-9/+_.~-]{16,}={0,2})\3`,
    'gi',
  ),
];
  ...
  return {
    redact(text: string): string {
      ...
      for (const pattern of LABELED_SECRET_PATTERNS) {
        output = output.replace(pattern, (_match, label: string, separator: string, quote: string) => `${label}${separator}${quote}[REDACTED]${quote}`);
      }
