---
name: use-ai-handler
description: Integrate the ai-handler nugget into an app — install or vendor it, make chat/stream calls, resolve keys, request JSON output, and handle typed errors. Use when writing app code that calls AI model providers through this library, or when wiring up connections, key sources, policy, or telemetry.
---

# Using ai-handler in an app

`@jxburros/ai-handler` is a small, zero-dependency, isomorphic TypeScript library
...
1. **GitHub Packages (preferred).** Add a project `.npmrc`:
   ...
   ```
   then `npm install @jxburros/ai-handler`.
