...
export function defaultRetryable(kind) {
 ...
 */
function extractStructuredError(redactedBody) {
    if (!redactedBody || redactedBody.length > MAX_PARSEABLE_BODY_CHARS)
        return undefined;
    let parsed;
    try {
    ...
    if (typeof parsed !== 'object' || parsed === null)
        return undefined;
    const root = parsed;
    const errorObj = typeof root.error === 'object' && root.error !== null ? root.error : root;
...
export function fromUnknown(error, provider) {
