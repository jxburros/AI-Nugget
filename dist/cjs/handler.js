...
exports.resetPolicyWarningForTests = resetPolicyWarningForTests;
...
function resetPolicyWarningForTests() {
...
class AIHandler {
    ...
    queue = [];
    lastStarted = 0;
    sessionRedactor = new redact_js_1.SessionRedactor();
    policy;
    constructor(opts) {
        this.opts = opts;
        this.policy = opts.policy ?? (0, policy_js_1.allowAllPolicy)();
    ...
    async chat(conn, req) {
        ...
        if (!result)
            throw new errors_js_1.AIError('Call ended without a result', { kind: 'invalid_response', provider: conn.provider });
        return result;
    }
    async *stream(conn, req) {
        const callId = createCallId();
        const startedAt = Date.now();
        const adapter = (0, index_js_1.adapterFor)(conn.provider, conn.baseUrl);
        ...
        try {
            ...
            for (;;) {
                ...
                try {
                    ...
                    throw new errors_js_1.AIError('Provider did not emit a done event', { kind: 'invalid_response', provider: conn.provider });
                }
                catch (errorValue) {
                    const error = (0, errors_js_1.fromUnknown)(errorValue, conn.provider);
                    ...
                    try {
                        await (0, util_js_1.sleep)(delayMs, req.signal);
                    }
                    catch (sleepError) {
                        recorded = true;
                        return yield* this.failStream(callId, conn, req, startedAt, attempt, (0, errors_js_1.fromUnknown)(sleepError, conn.provider));
                    }
                }
            }
        }
            ...
            recorded = true;
            return yield* this.failStream(callId, conn, req, startedAt, Math.max(1, attempt), (0, errors_js_1.fromUnknown)(errorValue, conn.provider));
        }
        finally {
            if (acquired)
                this.release();
            if (acquired && !recorded) {
                await this.recordFailure(callId, conn, req, startedAt, Math.max(1, attempt), new errors_js_1.AIError('Call canceled before completion', { kind: 'canceled', retryable: false, provider: conn.provider }));
            }
        }
    }
    /**
    ...
    async preflight(callId, conn, model, metadata) {
        ...
        if (!policyResult.allowed) {
            return { ok: false, error: new errors_js_1.AIError(policyResult.reason, { kind: 'policy_blocked', retryable: false, provider: conn.provider }) };
        }
        let resolved;
        try {
            ...
            return { ok: false, error: (0, errors_js_1.fromUnknown)(errorValue, conn.provider) };
        }
        try {
            const info = {
                callId,
            ...
            if (await this.opts.hooks?.beforeCall?.(info) === 'deny') {
                return { ok: false, error: new errors_js_1.AIError('Call denied by beforeCall hook', { kind: 'policy_blocked', retryable: false, provider: conn.provider }) };
            }
        }
        catch (errorValue) {
    ...
    async testConnection(conn) {
        try {
            return await this.runProbe(conn, '__testConnection__', async (resolved) => {
                ...
                if (health) {
                    if (!health.ok)
                        throw new errors_js_1.AIError(health.detail ?? 'Connection failed', { kind: 'network', retryable: false, provider: conn.provider });
    ...
    async chatParsed(conn, req, schema) {
        ...
        if (validated.ok)
            return { data: validated.value, result };
        throw new errors_js_1.AIError(`Model output failed schema validation: ${validated.message}`, { kind: 'invalid_response', provider: conn.provider });
    }
    /**
    ...
    async embed(conn, req) {
        ...
        const adapter = (0, index_js_1.adapterFor)(conn.provider, conn.baseUrl);
        if (!adapter.embed) {
            return fail(new errors_js_1.AIError(`Provider ${conn.provider} does not support embeddings`, { kind: 'invalid_request', retryable: false, provider: conn.provider }));
        }
        await this.acquire(req.signal);
        try {
            ...
            return fail((0, errors_js_1.fromUnknown)(errorValue, conn.provider));
        }
        finally {
            this.release();
        }
    ...
    async resolveConnection(conn) {
        const profile = (0, profiles_js_1.profileFor)(conn.provider, conn.baseUrl);
        const baseUrl = conn.baseUrl ?? profile.defaultBaseUrl;
        if (!baseUrl)
            throw new errors_js_1.AIError(`Provider ${conn.provider} requires baseUrl`, { kind: 'invalid_request', retryable: false, provider: conn.provider });
        const keyRef = conn.keyRef ?? { kind: 'none' };
        const key = await this.opts.keySource.resolve(keyRef);
        if (!key.ok) {
            throw new errors_js_1.AIError(`API key unavailable: ${key.reason}`, { kind: 'key_unavailable', retryable: false, provider: conn.provider });
        }
        ...
        return {
            ...
            apiKey: key.apiKey,
            headers: (0, profiles_js_1.applyAuth)(profile, key.apiKey, conn.headers ?? {}),
        };
    }
    async acquire(signal) {
        ...
        while (this.active >= max) {
            await new Promise((resolve, reject) => {
                ...
                const onAbort = () => {
                    ...
                    reject(new errors_js_1.AIError('Call canceled while waiting for concurrency slot', { kind: 'canceled', retryable: false }));
                };
                if (signal?.aborted) {
                    reject(new errors_js_1.AIError('Call canceled while waiting for concurrency slot', { kind: 'canceled', retryable: false }));
                    return;
        ...
        try {
            if (wait)
                await (0, util_js_1.sleep)(wait, signal);
        }
        catch (error) {
            this.release();
    ...
    async recordSuccess(callId, conn, req, startedAt, attempts, result) {
        ...
        await this.record({
            ...
            promptChars: (0, util_js_1.promptChars)(req.messages),
    ...
    redactedError(error) {
        return new errors_js_1.AIError(this.redact(error.message), {
            kind: error.kind,
            status: error.status,
...
function withIdempotencyKey(resolved, callId) {
    const profile = (0, profiles_js_1.profileFor)(resolved.provider, resolved.baseUrl);
