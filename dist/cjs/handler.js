...
class AIHandler {
    ...
    async *stream(conn, req) {
        ...
        if (!policyResult.allowed) {
            const error = new errors_js_1.AIError(policyResult.reason, { kind: 'policy_blocked', retryable: false, provider: conn.provider });
        ...
        try {
            if (await this.opts.hooks?.beforeCall?.(info) === 'deny') {
                const error = new errors_js_1.AIError('Call denied by beforeCall hook', { kind: 'policy_blocked', retryable: false, provider: conn.provider });
                await this.recordFailure(callId, conn, req, startedAt, 1, error);
                yield { type: 'error', error: this.redactedError(error) };
                return;
            }
            ...
            for (;;) {
                ...
                try {
                    ...
                    try {
                        ...
                        const canceled = (0, errors_js_1.fromUnknown)(sleepError, conn.provider);
    ...
    async embed(conn, req) {
        ...
        const policyResult = this.policy.checkModel(conn.provider, req.model);
        if (!policyResult.allowed)
            return fail(new errors_js_1.AIError(policyResult.reason, { kind: 'policy_blocked', retryable: false, provider: conn.provider }));
        let resolved;
        try {
            resolved = await this.resolveConnection(conn);
        }
        ...
        try {
            const info = { callId, connection: conn, provider: conn.provider, model: req.model, metadata: req.metadata, resolved: safeResolved(resolved) };
            if (await this.opts.hooks?.beforeCall?.(info) === 'deny') {
                return fail(new errors_js_1.AIError('Call denied by beforeCall hook', { kind: 'policy_blocked', retryable: false, provider: conn.provider }));
            }
        }
        catch (errorValue) {
            return fail((0, errors_js_1.fromUnknown)(errorValue, conn.provider));
    ...
    async runProbe(conn, operation, action) {
        ...
        try {
            ...
            if (await this.opts.hooks?.beforeCall?.(info) === 'deny') {
                throw new errors_js_1.AIError('Call denied by beforeCall hook', { kind: 'policy_blocked', retryable: false, provider: conn.provider });
            }
