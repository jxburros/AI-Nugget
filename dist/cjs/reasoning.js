...
exports.createReasoningStripper = createReasoningStripper;
exports.stripReasoningBlocksDetailed = stripReasoningBlocksDetailed;
exports.stripReasoningBlocks = stripReasoningBlocks;
exports.containsReasoningBlock = containsReasoningBlock;
const TAG_PAIRS = [
...
function partialTagSuffixLength(text) {
    ...
    for (let len = maxLen; len >= 1; len -= 1) {
        ...
        for (const pair of TAG_PAIRS) {
            if (pair.open.startsWith(tail) || pair.close.startsWith(tail))
                return len;
        }
    }
    return 0;
}
function closesOwnLine(text, idx) {
    let i = idx - 1;
...
function createReasoningStripper() {
    ...
    function process(input, flushAll) {
...
function stripReasoningBlocksDetailed(text) {
    if (!text)
        return { visible: '', reasoning: '' };
...
function containsReasoningBlock(text) {
    const lower = text.toLowerCase();
